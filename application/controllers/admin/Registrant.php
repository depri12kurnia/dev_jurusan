...
class Registrant extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['groups'] = $this->M_registrant->get_groups();
        $data['title'] = 'Registrant | Admin Panel';
        $data['content'] = 'paneladmin/registrant/list';
    ...
    public function ajax_list()
    {
        ...
        $list = $this->M_registrant->get_datatables();
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $user) {
            ...
            $row[] = '<a class="btn btn-primary btn-sm" href="javascript:void(0)" title="Edit" onclick="edit_user(' . "'" . $user->id . "'" . ')"><i class="fa fa-edit"></i></a>
                      <a class="btn btn-danger btn-sm" href="javascript:void(0)" title="Delete" onclick="reset_password(' . "'" . $user->id . "'" . ',' . "'" . $user->username . "'" . ')"><i class="fa fa-key"></i></a>';
            $data[] = $row;
        }

        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->M_registrant->count_all(),
            "recordsFiltered" => $this->M_registrant->count_filtered(),
            "data" => $data,
    ...
    {
        $data = $this->M_registrant->get_by_id($id);
    ...
    public function ajax_delete($id)
    {
        ...
        $this->M_registrant->delete_user_groups($id);
    ...
    private function _validate()
    {
        ...
        if (empty($username)) {
            ...
            $data['status'] = FALSE;
        } elseif ($this->M_registrant->is_username_exists($username)) {
            $data['inputerror'][] = 'username';
    ...
    {
        $this->validate_csrf(); // Pastikan CSRF valid

        $user_id = $this->input->post('user_id');
        $new_password = $this->input->post('new_password');

        // Pastikan admin yang melakukan reset
        if (!$this->ion_auth->is_admin()) {
            echo json_encode([
                'status' => FALSE,
        ...
        // Reset password dengan Ion Auth
        if ($this->ion_auth->update($user_id, ['password' => $new_password])) {
            // Simpan log aktivitas reset password
            $admin = $this->ion_auth->user()->row();
            $this->M_log_user->save_log($admin->id, "Reset password untuk user ID: $user_id");
